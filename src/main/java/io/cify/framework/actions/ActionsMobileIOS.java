...
public class ActionsMobileIOS implements IActions {

    @Override
    @Title("Open iOS browser")
    ...
    public void switchToWebView() {
        ...
        for (Object context : ((IOSDriver) getDevice().getDriver()).getContextHandles().toArray()) {
            if (context.toString().contains("WEBVIEW")) {
                ((IOSDriver) getDevice().getDriver()).context(context.toString());
                return;
            }
        }
    }
